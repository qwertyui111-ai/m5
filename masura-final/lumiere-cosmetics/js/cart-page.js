  ...
  return `
    <div class="cart-item" data-id="${p.id}">
      ...
      <div class="cart-item__info">
        <p class="cart-item__category">${p.category || ''}</p>
        <h3 class="cart-item__name">${p.name}</h3>
        <div class="cart-item__qty">
          <button class="qty-btn" data-action="decrease" data-id="${p.id}" aria-label="Уменьшить">−</button>
          <span class="qty-display" aria-live="polite">${item.qty}</span>
          <button class="qty-btn" data-action="increase" data-id="${p.id}" aria-label="Увеличить">+</button>
        </div>
      </div>
      <div class="cart-item__right">
        <span class="cart-item__price">${Number(p.price * item.qty).toLocaleString('ru-RU')} ₽</span>
        <button class="cart-item__remove" data-id="${p.id}" aria-label="Удалить">
  ...
  return `
    <aside class="order-summary" aria-label="Итог заказа">
      ...
      <div class="summary-row">
        ...
        <span>${Number(sub).toLocaleString('ru-RU')} ₽</span>
      </div>
      <div class="summary-row">
        <span>Доставка</span>
        <span>${delivery === 0 ? '<span style="color:var(--color-accent)">Бесплатно</span>' : Number(delivery).toLocaleString('ru-RU') + ' ₽'}</span>
      </div>
      ${delivery > 0 ? `<p style="font-size:12px;color:var(--color-text-muted);margin:4px 0 0">Бесплатно от 5 000 ₽</p>` : ''}
      ...
      <button class="btn btn-accent checkout-btn" onclick="handleCheckout()">Оформить заказ</button>
